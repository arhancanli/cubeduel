...
import { FLIP_COUNT, SLICE_COUNT, getFlip, getSlice, getTwist } from "./coords";
import { phase1Distance } from "./phase1Table";
...
test("both phase-one bounds solve the same cubes, and the exact one is not slower", () => {
  // Whether the exact table is on disk varies by machine — it is generated, not
  // committed — so both paths are run on purpose here. The fallback is not a
  // lesser answer: it is the same search under a weaker bound, and it must
  // still solve every cube it is given. Only the work differs.
  let exactNodes = 0;
  let pairNodes = 0;
  let compared = 0;
  ...
  for (let seed = 1; seed <= 12; seed++) {
    ...
    const withTable = solve(cube, { targetLength: 20, timeBudgetMs: 400 });
    const withoutTable = solve(cube, { targetLength: 20, timeBudgetMs: 400, exactTable: false });

    assert.ok(withTable, `seed ${seed} unsolved with the table`);
    assert.ok(withoutTable, `seed ${seed} unsolved without it`);
    assert.ok(applies(cube, withTable.moves), `seed ${seed}: the table's answer does not solve it`);
    assert.ok(applies(cube, withoutTable.moves), `seed ${seed}: the fallback's answer does not solve it`);

    if (!buildTables().phase1) continue;
    compared++;
    exactNodes += withTable.nodes;
    pairNodes += withoutTable.nodes;
  }

  // Only meaningful where the table exists. Where it does, the whole point is
  // that knowing the real distance visits fewer nodes for the same answers —
  // if that ever stopped being true, the table would be 67MB for nothing.
  if (compared > 0) {
    assert.ok(
      exactNodes < pairNodes,
      `the exact table visited ${exactNodes} nodes against the pair tables' ${pairNodes}`,
    );
  }
});

test("the exact table, when loaded, is the distance the search is told it is", () => {
  const t = buildTables();
  if (!t.phase1) return; // Generated by `npm run tables:build`; verified there.

  // A bound that is too high loses solutions silently, so this checks the
  // direction that matters: the exact distance is never below what the pair
  // tables — themselves exact for their own projections — say is needed.
  for (let seed = 1; seed <= 20; seed++) {
    const cube = randomState(seed * 7723);
    const twist = getTwist(cube);
    const flip = getFlip(cube);
    const slice = getSlice(cube);

    const exact = phase1Distance(t.phase1, twist, flip, slice);
    ...
    assert.ok(exact >= pairs, `seed ${seed}: exact ${exact} below the pair bound ${pairs}`);
    assert.ok(exact <= 12, `seed ${seed}: phase one cannot need ${exact} moves`);
  }

  assert.equal(phase1Distance(t.phase1, 0, 0, 0), 0, "solved is no distance at all");
});
